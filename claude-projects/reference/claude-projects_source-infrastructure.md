...
`generate_digest.py` writes the formatted digest to `digests/digest_latest.md`. In full-tier deployments (Claude Code, Codex), the orchestrator reads this file at session start and surfaces ACT NOW signals relevant to the active engagement.

In this lite tier, paste the digest content at session start so The Researcher can integrate it before intake.

Optional email delivery via Resend is supported in extended deployments. See `SERVICES_AND_KEYS.md` for setup.
